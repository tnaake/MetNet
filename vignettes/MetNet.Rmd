...
`addSpectSimil` uses functionality from the `Spectra` package, in order to 
create a spectral similarity matrix using MS2 data and adds it to the previously 
generated `structural` `AdjacencyMatrix`. 
It requires a `Spectra` object as input, storing the MS2 information and 
matching Feature ID's in the `spectra$id` slot. The "id" (see 
`spectraVariables()`)needs to be unique, meaning there is only one 
(representative) MS2 spectrum per feature. If features store multiple spectra, 
spectra consolidation has to be performed first (e.g. using the function 
`Spectra::combineSpectra()`).

Features from `spectra` will by matched accordingly to the feature names in the 
MS1 data (i.e. colnames/rownames in the `structural` `AdjacencyMatrix`).
...
parameters take a look on the respective help pages of 
`Spectra::compareSpectra`.
...
spect_adj <- addSpectSimil(spectra = sps_sub, 
...
Further, the spectral similarity matrix can be thresholded using the function threshold. 
The user needs to define wheather the data should be thresholded or not 
If multiple methods have been used to generate the spectral matrices, different threshold parameters can be defined (for detailed description see thresholding of statstical)
...
comb_adj <- combine(am_structural = spect_adj_thr, am_statistical = stat_adj_mean)
```
