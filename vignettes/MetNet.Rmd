...
adjacency matrix from structural properties, based on losses/addition of
...
matrix based on structural properties (m/z values) of the features.
...
returned adjacency matrix (value 1). Together with the adjacency matrix the type of
connection (derived from the column `"name"` in the
`transformation` object) will be written to a character matrix. These
two matrices will be returned as a list by the function
`structural`: the first entry contains the numerical adjacency
matrix and the second entry the character matrix.

Before calculating the structural matrix, one must define the search space, 
i.e. the transformation that will be looked for in the mass spectrometric
...
By default, an undirected structural adjacency matrix is directed (the
argument is set to `directed = FALSE` by default). `MetNet`,
...
g_undirected <- igraph::graph_from_adjacency_matrix(struct_adj[[1]], 
    mode = "directed", weighted = NULL)
plot(g_undirected, edge.width = 1, edge.arrow.size = 0.5,
     vertex.label.cex = 0.5, edge.color = "grey")
g_directed <- igraph::graph_from_adjacency_matrix(struct_adj_dir[[1]], 
    mode = "directed", weighted = NULL)
...
## Advanced topic: Refining the structural adjacency matrix (optional) {-}
...
will create an unweighted adjacency matrix from the weighted adjacency matrices
...
`args` argument for each respective statistical model. Values above or below 
...
matrix. The unweighted adjacency matrices will be passed to the  
`consensus` function from the `sna` package [@Butts2016] 
...
`threshold` implements four ways to obtain an unweighted adjacency
matrix. We will create here for all ways the unweighted consensus adjacency 
matrices.
...
args_top <- list(n = 100)
## type = "top1" 
stat_adj_top1 <- threshold(statistical = stat_adj_l, type = "top1", 
    args = args_top)
...
they are greater or equal than `threshold` and the value 1 or 0 will be returned,
...
`type = "mean"` to combine it with the structural adjacency matrix, `stat_adj`: 
