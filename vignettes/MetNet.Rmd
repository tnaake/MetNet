...
`"mass"`, `"name"` and additional columns (e.g. `"formula"` or `"rt"`). 
...
Together with this assay, an adjacency matrix with the type of
connection/transformation (derived from the column `"name"` in the
`transformation` object) will be written as a character matrix to the 
assay `"transformation"`. Furthermore, the mass difference will be written
as a character matrix to the assay (`"mass_difference"`).
...
with updated adjacency matrices (`"binary"`, `"transformation"`, and 
`"mass_difference"`). 
...
Some overview on the mass-difference distribution of the data can be observed using the `mz_summary` function. For larger data-sets, also a `filter` can be applied to visualize mass-difference above a defined threshold.

```{r,eval=TRUE,echo=TRUE}
mz_summary(struct_adj)
```

The `AdjacencyMatrix` class allows storing further information on the features, as putative annotations, database identifier, SMILES, etc. using `rowData()`. A `data.frame` containing the same `rownames` as the test data needs to be provided. The columns can store different information as annotations, identifier, etc.
We will load the `x_annotation` file, which contains an example annotation and other identifier for feature x1856. All the other features contain `NA`'s in corresponding columns. 
...
`AdjacencyMatrix` object. Furthermore, the information on the type of 
transformation and the mass difference are returned in the 
assays `"combine_transformation"` and `combine_mass_difference`.
