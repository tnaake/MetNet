...
From the `spectral` `AdjacencyMatrix` object the function `threshold`
will create an `AdjacencyMatrix` object with the derived 
unweighted adjacency matrix from the weighted adjacency matrices
unifying the information present from all spectral models. This 
unweighted adjacency matrix is stored in the assay `"consensus"`.

Parameters can be used, similarly as for `statistical` 
`AdjacencyMatrix` objects. 

```{r threshold,eval=TRUE,echo=TRUE}
## type = "threshold" 

## the assayNames in spec_adj are used to define the filter criteria
assayNames(spect_adj)

## return edges with a normal dotproduct > 0.1
...
## alternatively, use the types "top1", "top2", "mean"
## retrieve the feature pairs which have the 10 highest coefficients
args_top <- list(n = 10)
## type = "top1" 
spect_adj_top1 <- threshold(am = spect_adj, type = "top1", 
    args = args_top)

## for type = "top2" minimum two models have to be included in spect_adj
 
## type = "mean"
spect_adj_mean <- threshold(am = spect_adj, type = "mean", 
    args = args_top)
```



# Combining the structural, statistical and spectral matrix {#sec-combine}

After creating the `structural`, `spectral`  and `statistical` `AdjacencyMatrix` 
objects, two objects are combined. The function `combine` 
will combine two objects and create an `AdjacencyMatrix` object of type
`combine`. The function accepts the arguments `am_structural`, `am_structural` 
or `am_statistical` for the respective `AdjacencyMatrix` objects. Please note 
that for `am_structural` the `AdjacencyMatrix` obtained via `structural` or 
`rtCorrection` can be used, while for `am_statistical` and `am_spectral` 
the `AdjacencyMatrix` from `threshold` has to be used. 
...
comb_adj <- combine(am_1 = stat_adj_mean, am_2 = spect_adj_thr)
```
