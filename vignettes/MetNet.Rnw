...
properties of features \citep{Bretiling2006}. 
...
construction was first proposed in \citep{Breitling2006} and followed soon 
afterwards by a Cytoscape plugin, MetaNetter \citet{Jourdan2007}, that 
...
Inspired of the paper by \citep{Marbach2012} we decided to implement in 
...
calculation of a consensus network using the differently computed individual 
networks. \newline
...
Pearson and Spearman correlation (see \citep{Steuer2006} for a discussion on 
correlation-based metabolic networks), context likelihood of relatedness (CLR),
...
\code{log2} transformation, noise filtering, removal of features that do not represent
...
The function \Rfunction{create\_structural\_network} will create the adjacency
...
\Rfunction{create\_structural\_network} takes a \Robject{data.frame}
...
\Rfunction{create\_structural\_network} looks for transformation (in the 
...
Following the work of \citep{Breitling2006} and \citep{Jourdan2007}, 
...
As examplified in \citep{Jourdan2007} specific enzymatic reactions refer to 
...
\Rfunction{create\_structural\_network} allowing to look for specific 
enzymatic transformations in mind. Hereby,
\Rfunction{create\_structural\_network} will take into account the \Robject{ppm}
...
\Rfunction{create\_structural\_network}.
...
<<structure,eval=FALSE,echo=TRUE>>=
struct_adj <- create_structural_network(x = x_test, functional_groups = functional_groups, ppm = 3)
@


\section{Creating the statistical matrix}

The function \Rfunction{create\_statistical\_network} will create the adjacency
matrix based on statistical associations. \Rfunction{create\_statistical\_network}
is a wrapper function for the functions 
\Rfunction{create\_statistical\_networks\_list} and \Rfunction{consensus\_network}. 
The function \Rfunction{create\_statistical\_networks\_list} will create a 
...
and Spearman correlation and constraint-based structure learning (based on 
...
the \Rfunction{create\_statistical\_networks} and 
\Rfunction{create\_statistical\_networks\_list} functions. In addition, arguments 
...
From the list of adjacency matrices the function \Rfunction{consensus\_network}
...
are accepted by this function can be passed to the \Rfunction{consensus\_network}
and \Rfunction{create\_statistical\_networks} function, respectively. Furthermore, 
...
or not. \Rfunction{create\_statistical\_networks} and \Rfunction{consensus\_network}
...
<<statistical,eval=FALSE,echo=TRUE>>=
x_int <- x_test[,-3:dim(x_test)[2]]
stat_adj <- create_statistical_network(x_int, 
...
call the two functions \Rfunction{create\_statistical\_networks\_list} and 
\Rfunction{consensus\_network} individually: 
<<statistical_stepwise,eval=FALSE,echo=TRUE>>= 
...
% \Rfunction{create\_statistical\_networks}/\Rfunction{consensus\_network} is used. 

<<combine,eval=FALSE,echo=TRUE>>=
cons_adj <- combine_structural_statistical(structure = struct_adj[[1]],
...
<<visualisation,eval=FALSE,echo=TRUE,fig.show='hide'>>=
g <- igraph::graph_from_adjacency_matrix(matrix = cons_adj, mode = "directed", 
                           directed = FALSE)
plot(g)
@
...
(topological parameters such as centrality, degree, clustering indices) that are 
implemented in different packages in \R 
