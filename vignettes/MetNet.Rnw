...
\abstract{
A major bottleneck of mass spectrometry metabolomic analysis is 
...
We present here \Rpackage{MetNet}, a novel \R{} package, that is compatible 
...
Inspired of the paper by \cite{Marbach2012} we decided to implement in 
...
\CRANpkg{parmigene}, \CRANpkg{psych} and \CRANpkg{bnlearn} that are needed
...
transformations <- rbind(
    ...
    c("Rhamnose (-H20)", "C6H12O5", 146.057910, "-"),
...
\CRANpkg{psych} package, \cite{Revelle2018}), partial and semipartial 
...
\Rcode{randomForest}, \Rcode{clr}, \Rcode{aracne}, \Rcode{correlation} and/or 
\Rcode{bayes} can be passed like-wise to the functions. \newline
...
\begin{figure}[t!]
    ...
    \caption{\textbf{\textit{Ab inition} network inferred from structural 
