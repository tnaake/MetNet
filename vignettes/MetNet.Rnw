...
\citep{Breiman2001}, Pearson and Spearman correlation (see \citet{Steuer2006} 
...
will be combined to form a consensus matrix that has both information from
...
The function \Rfunction{create\_structural\_network} will then check for those 
...
\Rfunction{consensus\_network}. The former function will create a 
...
\Rpackage{psych} package; 
\citet{Revelle2018}) and constraint-based structure learning based on 
the Fast Incremental Association (Fast-IAMB, algorithm from the 
\Rpackage{bnlearn} package; \citet{Scutari2010}). 
...
l <- create_statistical_networks_list(x_int, 
                model = c("pearson", "spearman"), correlation_adjust = "BH")
