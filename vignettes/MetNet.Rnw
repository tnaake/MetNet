...
\Rfunction{create\_structural\_network} will take into account the 
...
struct_adj <- create_structural_network(x = x_test, 
...
\Rfunction{create\_statistical\_network} is a wrapper function for the 
functions \Rfunction{create\_statistical\_networks\_list} and 
\Rfunction{consensus\_network}. The function 
\Rfunction{create\_statistical\_networks\_list} will create a 
...
the Fast Incremental Association, Fast-IAMB, algorithm from the 
\Rpackage{bnlearn} package). For furthe information on the different models 
take a look on the respective help pages of \code{lasso}, \code{randomForest}, 
\code{clr}, \code{.aracne}, \code{?correlation} and/or \code{bayes}. Functions 
that are accepted by the respective underlying functions can be passed directly 
to the \Rfunction{create\_statistical\_networks} and 
\Rfunction{create\_statistical\_networks\_list} functions. In addition, 
...
are accepted by this function can be passed to the 
\Rfunction{consensus\_network} and \Rfunction{create\_statistical\_networks} 
...
the value $a_{i,j}$ of the consensus matrix 
will be reported as a connection in the returned matrix 
(if $a_{i,j} \geq$ \code{threshold})
or not. \Rfunction{create\_statistical\_networks} and 
\Rfunction{consensus\_network} will return an unweighted adjancency matrix 
...
l <- create_statistical_networks_list(x_int, model = c("pearson", "spearman"),  
                                                    adjust_correlation = "BH")
...
\Rfunction{create\_statistical\_networks}/\Rfunction{consensus\_network} is 
used. 
