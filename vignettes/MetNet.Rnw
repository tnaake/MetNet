...
<<>>=
struct_adj <- create_structural_matrix(x, functional_groups, ppm = 5)
@
...
<<statistical>>=
stat_adj <- create_statistical_network(x, model = c("pearson", "spearman"), adjust_correlation = "BH")
@
...
<<visualisation,eval=TRUE>>=
cons_adj_g <- graph_fromadjacency_matrix(matrix = cons_adj, mode = "directed", 
                           directed = FALSE)
plot(cons_adj_g)
@
