...
(LASSO, L1-norm regression, \citet{Tibshirani1994}), Random Forest 
...
of relatedness (CLR, \citealp{Faith2007}), the algorithm for the reconstruction 
of accurate cellular networks (ARACNE, \citealp{Margolin2006}) and 
constraint-based structure learning (Bayes, \citealp{Scutari2010}). 
...
object as argument \Robject{transformations} with the \code{colnames}
...
\code{"mass"}es of the \Robject{transformations} argument. If any
...
struct_adj <- createStructuralAdjacency(x=x_test, 
                                transformation=transformations, ppm=3)
@
in the \R~console. 
...
argument (in the \code{"rt"} column). Within this columns, information about
retention time shifts is encoded by \code{"-"}, \code{"+"} and \code{"?"}, 
...
\citealt{Hofner2015,Thomas2017}), Random Forest (using 
...
\Rpackage{psych} package; \citealt{Revelle2018}), partial and semipartial 
...
from the \Rpackage{bnlearn} package; \citealt{Scutari2010}). 
...
The list of transformations is taken from \citet{Breitling2006}. 
